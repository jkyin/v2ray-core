...
build_v2() {
	...
	# go build -tags ios -prefix V2Ctl -o $HOME/v2ctl -buildmode=c-archive -ldflags "-s -w" -x -v ./infra/control/main

	# env CGO_ENABLED=0 go build -o $TMP/v2ctl${EXESUFFIX} -tags confonly -ldflags "$LDFLAGS" ./infra/control/main

	gomobile bind -target=ios -prefix=V2Ray -ldflags "$LDFLAGS" -x -v v2ray.com/core
